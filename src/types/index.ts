...
export interface ScoreFactors {
  er: number; sr: number; feas: number; cov: number; damp: number; gate: number;
  contrib_er: number; contrib_sr: number; contrib_feas: number; contrib_cov: number;
  raw: number;
...
export interface NetworkSummary {
  as_of: string;
  total_units: number; total_lots: number;
  at_risk_units: number; rescuable_units: number;
  facilities_with_gap: number; total_gap_units: number;
  recommendation_count: number; exclusion_count: number;
...
export interface AlertItem {
  id: string;
  type: "EXPIRY" | "SHORTAGE" | "ANOMALY" | "INFO";
  title: string;
  message: string;
