...
export const NetworkPage: React.FC = () => {
  ...
  const [activeFilter, setActiveFilter] = useState<'ALL' | 'GOLDEN' | 'HIGH_PRIORITY' | 'O_POSITIVE'>('ALL');
  const [hoveredNodeId, setHoveredNodeId] = useState<string | null>(null);

  // Optimized topological layout to prevent line crossing:
  // Sources/Surplus on Left (West), Hub in Center, Destinations/Deficits on Right (East)
  const OPTIMIZED_LOCATIONS: Record<string, { x: number; y: number }> = {
    'h1': { x: 170, y: 150 }, // Greenfield Medical Center (Source - Top Left)
    'h2': { x: 630, y: 160 }, // City General Hospital (Destination - Top Right)
    'h3': { x: 620, y: 60 },  // Apollo Care Hospital (Top Far Right)
    'h4': { x: 380, y: 240 }, // Metro Blood Centre (Central Hub)
    'h5': { x: 180, y: 390 }, // St. Mary's Medical Institute (Bottom Left)
    'h6': { x: 130, y: 270 }, // Sunrise Hospital (Mid Left)
    'h7': { x: 640, y: 380 }, // Central Trauma Centre (Bottom Right)
    'h8': { x: 260, y: 60 },  // Lakeside Medical Center (Top Mid-Left)
    'h9': { x: 720, y: 270 }, // Unity Healthcare (Far Right)
    'h10': { x: 440, y: 70 }, // North District Hospital (Top Mid-Right)
  };

  // Compute status for each hospital node
  const getHospitalState = (hId: string) => {
    if (hId === 'h2') return { state: 'SHORTAGE_RISK', color: '#ef4444', label: 'Deficit Risk (-5u)' };
    if (hId === 'h1') return { state: 'EXPIRY_RISK', color: '#f59e0b', label: 'Expiry Risk (7u)' };
    if (hId === 'h7' || hId === 'h10') return { state: 'SHORTAGE_RISK', color: '#ef4444', label: 'Critical Deficit' };
    if (hId === 'h4' || hId === 'h6') return { state: 'WATCH', color: '#eab308', label: 'Watch Reserve' };
  ...
  const nodes = useMemo(() => {
    return hospitals.map((h) => {
      const invCount = inventory.filter((i) => i.hospitalId === h.id).reduce((acc, i) => acc + i.quantity, 0);
      const loc = OPTIMIZED_LOCATIONS[h.id] || h.location;
      const { state, color, label } = getHospitalState(h.id);
      return {
        ...h,
        x: loc.x,
        y: loc.y,
  ...
  const filteredRecs = useMemo(() => {
    return recommendations.filter((rec) => {
      if (activeFilter === 'GOLDEN') return rec.id === 'HG-REC-001';
      if (activeFilter === 'HIGH_PRIORITY') return rec.priority === 'HIGH';
      if (activeFilter === 'O_POSITIVE') return rec.bloodGroup === 'O+';
      return true;
    });
  ...
  const edges = useMemo(() => {
    return filteredRecs.map((rec, index) => {
      const srcNode = nodes.find((n) => n.id === rec.sourceHospitalId);
      const dstNode = nodes.find((n) => n.id === rec.destinationHospitalId);
      ...
      // Quadratic bezier path string
      const pathD = `M ${x1} ${y1} Q ${cx} ${cy} ${x2} ${y2}`;

      // Label coordinate along bezier curve at t=0.5
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 bg-white p-1.5 rounded-full border border-slate-200/80 shadow-sm">
          ...
          <button
            onClick={() => setActiveFilter('O_POSITIVE')}
            className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${
              activeFilter === 'O_POSITIVE'
                ? 'bg-emerald-900 text-white shadow-sm'
                : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
            }`}
          >
        ...
        <div className="lg:col-span-3 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 rounded-3xl p-6 shadow-2xl relative min-h-[540px] flex items-center justify-center overflow-hidden border border-slate-800">
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 800 480">
            ...
            {edges.map((edge) => {
              ...
              const isExcluded = edge.status === 'EXCLUDED';
              const isHighlighted = hoveredNodeId === edge.sourceHospitalId || hoveredNodeId === edge.destinationHospitalId;
              ...
              return (
                ...
                >
                  ...
                  <path
                    d={edge.pathD}
                    fill="none"
                    stroke={isExcluded ? '#f43f5e' : isGolden ? '#10b981' : '#38bdf8'}
                    strokeWidth={isGolden ? 3.5 : 2}
                    strokeDasharray={isExcluded ? '6 6' : 'none'}
                    markerEnd={isExcluded ? 'url(#arrowhead-excluded)' : 'url(#arrowhead-active)'}
                  ...
                  >
                    <div className="flex items-center justify-center">
                      ...
                      >
                        {edge.bloodGroup} {edge.component} • {edge.quantity}u
                      </span>
                    </div>
                  </foreignObject>
                </g>
          ...
          <div className="absolute inset-0 w-full h-full pointer-events-none">
            {nodes.map((node) => {
              const isSelected = selectedHospital?.id === node.id;

              return (
                <div
                  key={node.id}
                  onClick={() => setSelectedHospital(node)}
          ...
          <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between text-[11px] text-slate-400 bg-slate-950/90 px-4 py-2 rounded-2xl border border-slate-800 backdrop-blur">
            <span className="flex items-center gap-2">
              ...
              <strong>Active Flow View:</strong> West (Surplus Sources) $\rightarrow$ East (Demand Destinations)
            </span>
        ...
        <div className="bg-white rounded-3xl p-6 shadow-card border border-slate-100 flex flex-col justify-between space-y-4">
          <div>
            ...
            <div className="p-4 bg-emerald-50/80 rounded-2xl border border-emerald-200/80 space-y-3 mb-4">
              ...
              <button
                onClick={() => {
                  const rec = recommendations.find((r) => r.id === 'HG-REC-001');
            ...
            {selectedHospital ? (
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2">
                ...
                <h4 className="font-extrabold text-sm text-slate-900">{selectedHospital.name}</h4>
                <p className="text-xs text-slate-500">{selectedHospital.tier} • {selectedHospital.city}</p>
                <div className="pt-2 flex items-center justify-between text-xs font-semibold border-t border-slate-200/60">
                  <span className="text-slate-600">Capacity:</span>
                  <span className="text-slate-900 font-bold">{selectedHospital.totalCapacity} Units</span>
                </div>
              </div>
            ) : (
