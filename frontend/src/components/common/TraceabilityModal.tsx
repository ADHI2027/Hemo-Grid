...
export const TraceabilityModal: React.FC = () => {
  const { selectedTraceabilityItem, setSelectedTraceabilityItem, setActiveRoute } = useApp();

  if (!selectedTraceabilityItem) return null;

  const item = selectedTraceabilityItem;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-200/80">
                Unit ID: {item.unitOrBatchId} • {item.bloodGroup} {item.component}
              </p>
            </div>
          </div>
          <button
            onClick={() => setSelectedTraceabilityItem(null)}
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="p-3.5 bg-slate-50 rounded-2xl border border-slate-100">
              ...
              <span className="text-sm font-semibold text-slate-800 truncate block mt-0.5">{item.hospitalName}</span>
            </div>
            <div className="p-3.5 bg-slate-50 rounded-2xl border border-slate-100">
              <span className="text-xs font-medium text-slate-500 block">Blood Group</span>
              <span className="text-sm font-bold text-emerald-900 block mt-0.5">{item.bloodGroup} {item.component}</span>
            </div>
            <div className="p-3.5 bg-slate-50 rounded-2xl border border-slate-100">
              <span className="text-xs font-medium text-slate-500 block">Quantity</span>
              <span className="text-sm font-semibold text-slate-800 block mt-0.5">{item.quantity} Units</span>
            </div>
            <div className="p-3.5 bg-slate-50 rounded-2xl border border-slate-100">
            ...
            <div className="relative pl-6 space-y-6 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
              ...
              <div className="relative flex items-start gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-400">{item.collectionDate}</span>
                  </div>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Donor batch reference {item.donorBatchRef || 'BATCH-2026-X90'}. Infectious disease screening completed & cleared.
                  </p>
                </div>
              </div>
              ...
              <div className="relative flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    Storage status: <span className="font-semibold text-slate-700">{item.storageStatus}</span> at {item.temperature}°C continuous telemetric monitoring.
                  </p>
                </div>
              </div>
              ...
              <div className="relative flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    Active location: {item.hospitalName}. Days remaining to expiry: <span className="font-bold text-amber-700">{item.daysToExpiry} days</span>.
                  </p>
                </div>
              </div>
